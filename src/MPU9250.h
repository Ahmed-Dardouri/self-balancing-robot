...
public:
    ...
    bool setup(const uint8_t addr, const MPU9250Setting& mpu_setting = MPU9250Setting(), WireType& w = Wire) {
        ...
        if ((addr < MPU9250_DEFAULT_ADDRESS) || (addr > MPU9250_DEFAULT_ADDRESS + 7)) {
            // .print("I2C address 0x");
            // .print(addr, HEX);
            // .println(" is not valid for MPU. Please check your I2C address.");
            return false;
        }
        mpu_i2c_addr = addr;
        setting = mpu_setting;
        ...
        if (isConnectedMPU9250()) {
            ...
            if (b_verbose)
                // .println("Could not connect to MPU9250");
            has_connected = false;
            return false;
        }
        has_connected = true;
...
private:
    ...
    void initAK8963() {
        ...
        if (b_verbose) {
            /* .println("Mag Factory Calibration Values: ");
             .print("X-Axis sensitivity offset value ");
             .println(mag_bias_factory[0], 2);
             .print("Y-Axis sensitivity offset value ");
             .println(mag_bias_factory[1], 2);
             .print("Z-Axis sensitivity offset value ");
             .println(mag_bias_factory[2], 2);*/
        }
    }
...
private:
    ...
    void collect_mag_data_to(float* m_bias, float* m_scale) {
        if (b_verbose)
            // .println("Mag Calibration: Wave device in a figure eight until done!");
        delay(4000);
        ...
        if (b_verbose) {
            /* .println("mag x min/max:");
             .println(mag_min[0]);
             .println(mag_max[0]);
             .println("mag y min/max:");
             .println(mag_min[1]);
             .println(mag_max[1]);
             .println("mag z min/max:");
             .println(mag_min[2]);
             .println(mag_max[2]);*/
        }
